...
func main() {
	...
	result = db.Model(entities.Company{}).Count(&companiesCount) // SELECT COUNT(*) FROM companies;
	if result.Error != nil {
		log.Fatal(result.Error) // returns error
	}
	...
	//Get all projects
	result = db.Preload(clause.Associations).Find(&projects) // SELECT * FROM projects;
	if result.Error != nil {
		log.Fatal(result.Error) // returns error
	}
