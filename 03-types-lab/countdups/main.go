...
func main() {
	...
	wordCounts := make([]Entry, len(counts))
	i := 0
	for word, count := range counts {
		wordCounts[i] = Entry{Word: word, Count: count}
		i++
	}

	fmt.Printf("Word: %v", wordCounts)

	sort.Slice(wordCounts, func(i, j int) bool { return wordCounts[i].Count > wordCounts[j].Count })

	for i, val := range wordCounts {
		fmt.Printf("%d: %-20.20s -> %5d\n", i, val.Word, val.Count)
		if i > 10 {
			break
		}
	}
