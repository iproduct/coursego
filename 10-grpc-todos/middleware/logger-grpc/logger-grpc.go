...
var (
	// Log is global logger-grpc
	Log *zap.Logger

	// timeFormat is custom Time format
	customTimeFormat string

	// onceInit guarantee initialize logger-grpc only once
	onceInit sync.Once
)
...
// timeFormat - custom time format for logger-grpc of empty string to use default
func Init(lvl int, timeFormat string) error {
	...
	onceInit.Do(func() {
		...
		if !useCustomTimeFormat {
			Log.Warn("time format for logger-grpc is not provided - use zap default")
		}
	})
